...
Before writing custom code to solve a problem, check if an npm package already exists.
This saves time, reduces bugs, and leverages community-tested solutions.

## When to use this skill

   * The project has a package.json file and uses NPM/Node
   * User asks for functionality that sounds like a common problem (date formatting,
     validation, HTTP requests, etc.)
...
The `--long` flag shows additional details including description, author, date, version,
and keywords.
...
1. **Popularity** - Higher download counts generally indicate reliability and community
   support
